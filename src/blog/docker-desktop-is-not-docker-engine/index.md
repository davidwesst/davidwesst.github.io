...
To be clear, I like Docker for Desktop. It provides me an easy-to-use GUI (graphical user interface) to manage my docker images I use for various tasks for building software. I use it on Windows, and now I'll be using it on Linux as [it is available for some of the the more common distros][1]. As great as it is, it is different than running [Docker Engine][2] right on the metal (rather than a virtual machine, like Docker of Desktop), and some of those differences caught me while testing my GitHub Workflows with [nektos/act][3], which depends on Docker Engine to work.

To be fair, this is documented right in the [README for the nektos/act][4], and had I reviewed the documentation I probably would have saved myself the trouble. Still, I in my web sleuthing for solutions, I found others hit similar issues, hence this post.
...
![Docker documentation page with a banner highlighting that Docker for Desktop now exists for Linux](#)
...
This is where things went sideways and the problem appeared. I ran `act -j build` to run a workflow I know has worked previously and was greeted with the following error message:
...
![Screenshot of a Linux terminal showing the Docker CLI output for docker context list command that lists the default docker context, which is the Docker Engine context, and the Docker for Desktop context for the user](#).
